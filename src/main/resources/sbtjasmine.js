...
function runTests(appJsRoot, appJsLibRoot, testRoot, confFile, envHtml) {
    ...
    for (i = 0, len = EnvJasmine.specs.length >>> 0; i < len; i += 1) {
        try {
            ...
            var windowsPreparedEnvHTML = envHtml.split("\\").join("\\\\");
            print(windowsPreparedEnvHTML);
            var windowLoader = 'window.location.assign(["file://", "'+windowsPreparedEnvHTML+'"].join(EnvJasmine.SEPARATOR));';
